...
def plot_fig(A_recent, config, T0):
    ...
    m_map, inmap = plotting.make_map(ax, lat0, lon0, main_dist=50, inset_dist=500, scale=15)

    try:
        volcs = pd.read_excel(config.volc_file)
        volcs = processing.volcano_distance(lon0, lat0, volcs)
    ...
    jpg_file = plotting.save_file(plt, config, dpi=300)
